...
import { users, goals, routines, routineWindows, days, blocks } from "@/lib/schema";
...
function instantiateFromRoutine(items: Array<{ startMin: number; endMin: number; depthLevel: Depth; goalId: number | null; label?: string | null }>) {
  return items
    .slice()
...
export async function GET(req: NextRequest, ctx: Ctx) {
  ...
  if (slug[0] === "routine" && slug[1] === "status") {
    // keep this lightweight (cookie-based so no DB migration needed)
  ...
  if (slug[0] === "routine") {
    ...
    const [win] = await db.select().from(routineWindows).where(and(eq(routineWindows.userId, uid), eq(routineWindows.weekday, weekday)));
  ...
  if (slug[0] === "day") {
    ...
    const pack = {
      ...
      blocks: blks.map((b) => ({
        ...
        cognition: b.depthLevel as Depth,
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  if (slug[0] === "goals") {
    ...
    const [g] = await db.insert(goals).values({ userId: uid, label, color, deadlineISO: deadlineISO ?? null }).returning();
  ...
  if (slug[0] === "routine" && slug[1] === "conflicts") {
    ...
    const items: Array<{ days: number[]; sprints: Array<{ startMin: number; endMin: number }> }> =
    ...
    const existing = await db.select().from(routines).where(and(eq(routines.userId, uid), inArray(routines.weekday, daysList)));
  ...
  if (slug[0] === "routine" && slug[1] === "push") {
    ...
    // items: [{ label, depthLevel, days: number[], sprints: [{startMin,endMin}] }]
    const items: Array<{ label?: string; depthLevel: number; days: number[]; sprints: Array<{ startMin: number; endMin: number }> }> =
      Array.isArray(body?.items) ? body.items : [];
    ...
    if (!overwrite) {
      const chk = await POST(
        new NextRequest(new URL(req.url).toString(), {
          method: "POST",
          body: JSON.stringify({ items: items.map((it) => ({ days: it.days, sprints: it.sprints })) }),
          headers: { "content-type": "application/json" },
        } as any),
        { params: Promise.resolve({ slug: ["routine", "conflicts"] }) } as any
      );
      const cj = await chk.json();
      if ((cj?.conflicts?.length ?? 0) > 0) return NextResponse.json({ error: "conflicts", conflicts: cj.conflicts }, { status: 409 });
    }
      ...
      if (daysList.length) {
        ...
        for (const it of items) {
          for (const wd of it.days) {
            ...
            for (const sp of it.sprints) {
              for (const r of ex) {
                if (overlap(sp.startMin, sp.endMin, r.startMin, r.endMin)) toDeleteIds.push(r.id);
              }
            }
          }
        }
    ...
    for (const it of items) {
      ...
      const depth: Depth = (it.depthLevel === 1 || it.depthLevel === 2 || it.depthLevel === 3) ? it.depthLevel as Depth : 3;
      for (const wd of it.days) {
        ...
        const values = it.sprints.map((s, i) => ({
          ...
          endMin: s.endMin,
          depthLevel: depth,
          goalId: null as any,   // goalless routine
          label,
          orderIndex: i,
        }));
        if (values.length) {
  ...
  // legacy single-day routine upsert (kept for back-compat)
  if (slug[0] === "routine") {
    const { weekday, items, applyTo, window } = body || {};

    // Bulk apply to many days (preferred)
    if (Array.isArray(applyTo) && applyTo.length > 0) {
      const wds = applyTo.map(Number);
      if (wds.some((wd) => !(wd >= 0 && wd <= 6))) {
        ...
        if (Array.isArray(items) && items.length) {
          await db.insert(routines).values(
            items.map((x: any, i: number) => ({
              ...
              goalId: (x.goalId ?? null) as any,
              label: typeof x.label === "string" && x.label.trim() ? x.label.trim() : null,
              orderIndex: Number.isFinite(x.orderIndex) ? x.orderIndex : i,
            })),
          );
        }
      }
    ...
    // Back-compat: single weekday mode
    if (!(Number(weekday) >= 0 && Number(weekday) <= 6)) {
      return NextResponse.json({ error: "weekday 0..6 required (or provide applyTo[])" }, { status: 400 });
    }

    if (window && Number.isInteger(window.openMin) && Number.isInteger(window.closeMin)) {
      await db.delete(routineWindows).where(and(eq(routineWindows.userId, uid), eq(routineWindows.weekday, Number(weekday))));
      await db.insert(routineWindows).values({
        userId: uid,
    ...
    await db.delete(routines).where(and(eq(routines.userId, uid), eq(routines.weekday, Number(weekday))));
    if (Array.isArray(items) && items.length) {
      await db.insert(routines).values(
        items.map((x: any, i: number) => ({
          userId: uid,
