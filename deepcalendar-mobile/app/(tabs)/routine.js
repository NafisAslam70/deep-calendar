  ...
  return (
    <Pressable
      ...
      style={{
        ...
        borderWidth: 1,
        borderColor: "#e5e7eb",
        backgroundColor: on ? "#111827" : "white",
      }}
    >
...
function Btn({ title, onPress, kind = "primary" }) {
  ...
  return (
    <Pressable
      onPress={onPress}
      style={{ backgroundColor: bg, borderWidth: 1, borderColor, paddingVertical: 12, paddingHorizontal: 16, borderRadius: 10 }}
    >
...
export default function RoutineBuilder() {
  ...
  async function loadBase() {
    const gs = await api.get("/api/deepcal/goals");
    setGoals(gs?.goals || []);
    ...
    res.forEach((r, idx) => {
      w[idx] = r.window || null;
      e[idx] = (r.items || []).sort((a, b) => a.startMin - b.startMin);
    });
    setWins(w);
    setExisting(e);
  }
  ...
  useEffect(() => {
    ...
    loadBase().catch((e) => Alert.alert("Error", e?.message || String(e)));
  }, [authLoading, user]);
  ...
  async function applyWindow() {
    const days = Object.entries(wDays)
      .filter(([, on]) => on)
      .map(([d]) => Number(d));
    const openMin = toMinutes(wOpen),
      closeMin = toMinutes(wClose);
  ...
  function addBreak() {
    ...
    setBreaks([...breaks, { s, e }].sort((a, b) => a.s - b.s));
    setBrStart("");
    setBrEnd("");
  }
  ...
  async function addBlock() {
    ...
    const days = Object.entries(daySel)
      .filter(([, on]) => on)
      .map(([d]) => Number(d));
    if (!days.length) return Alert.alert("No days", "Choose at least one day");

    // Simple warnings check against existing/window (kept same behavior)
    const warns = [];
    for (const d of days) {
      const win = wins[d];
      const items = existing[d] || [];
  ...
  async function pushDraft() {
    ...
    const doWrite = async () => {
      for (const [weekday, items] of byDay.entries()) {
        await api.post("/api/deepcal/routine", {
          applyTo: [weekday],
    ...
    <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
        <Text
          style={{
            ...
            paddingVertical: 4,
            borderRadius: 8,
            backgroundColor: finalized ? "#ecfdf5" : "#fffbeb",
            color: finalized ? "#065f46" : "#92400e",
          }}
        >
      ...
      <View style={{ borderWidth: 1, borderColor: "#e5e7eb", borderRadius: 12, padding: 12, gap: 8 }}>
        ...
        <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap" }}>
          ...
          />
          <TextInput
            placeholder="Depth 1|2|3"
            value={String(bDepth)}
            onChangeText={(v) => setBDepth(Number(v) || 1)}
            style={{ borderWidth: 1, borderColor: "#e5e7eb", borderRadius: 10, padding: 10, minWidth: 110 }}
          />
          <TextInput
            placeholder="Goal ID"
            value={String(bGoalId)}
            onChangeText={setBGoalId}
            style={{ borderWidth: 1, borderColor: "#e5e7eb", borderRadius: 10, padding: 10, minWidth: 110 }}
          />
        </View>
        ...
        <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap" }}>
          {WEEKDAYS.map((w, i) => (
            <Chip key={w} on={!!daySel[i]} label={w} onPress={() => setDaySel((s) => ({ ...s, [i]: !s[i] }))} />
          ))}
        </View>
        ...
        <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap" }}>
          <TextInput
            ...
            value={brStart}
            onChangeText={setBrStart}
            style={{ borderWidth: 1, borderColor: "#e5e7eb", borderRadius: 10, padding: 10, minWidth: 120 }}
          />
          <TextInput
            placeholder="Break end"
            value={brEnd}
            onChangeText={setBrEnd}
            style={{ borderWidth: 1, borderColor: "#e5e7eb", borderRadius: 10, padding: 10, minWidth: 120 }}
          />
      ...
      <View style={{ borderWidth: 1, borderColor: "#e5e7eb", borderRadius: 12, padding: 12, gap: 8 }}>
        ...
        {!draft.length ? (
          <Text style={{ color: "#6b7280" }}>No blocks yet.</Text>
        ) : (
          draft.map((g, idx) => (
            <View key={g.id} style={{ borderWidth: 1, borderColor: "#e5e7eb", borderRadius: 10, padding: 10, gap: 6 }}>
              ...
              {(g.sprints || []).map((sp, i) => (
                <Text key={i}>• {fromMinutes(sp.s)} – {fromMinutes(sp.e)}</Text>
              ))}
              <Text style={{ fontWeight: "600", marginTop: 4 }}>Breaks</Text>
              {!g.breaks.length ? (
                <Text style={{ color: "#6b7280" }}>No breaks</Text>
              ) : (
                g.breaks.map((br, i) => <Text key={i}>• {fromMinutes(br.s)} – {fromMinutes(br.e)}</Text>)
              )}
            </View>
          ))
        )}
